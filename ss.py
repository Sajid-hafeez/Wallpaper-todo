...
# main_font = load_font(36)
# attribution_font = load_font(24)  # Smaller font for attribution
#     except:
...
def load_font(size):
    try:
        return ImageFont.truetype("fonts/FreeSerif.ttf", size)
    except:
        return ImageFont.load_default()
...
def draw_attribution(draw, font, img, img_width, img_height):
    attribution_text = "Rprogrammers.com"
    # Calculate text position
    text_bbox = draw.textbbox((0, 0), attribution_text, font=font)
    text_width = text_bbox[2] - text_bbox[0]
...
def generate_wallpaper_with_todos(todos):
    ...
    # Draw Rprogrammers.com attribution
    draw_attribution(draw, attribution_font, img, img.width, img.height)
